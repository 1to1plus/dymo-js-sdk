...
export const DlsWebService = function () {
  ...
  this.renderLabel = function (
    labelXml, renderParamsXml, printerName) {
  ...
  this.loadImageAsPngBase64 = function (imageUri) {
    return invokeWsCommandAsync(GET, getSetting('WS_CMD_LOAD_IMAGE'),
      { imageUri })
  }

  this.is550Printer = function (printerName) {
    return invokeWsCommandAsync(GET, getSetting('WS_CMD_IS_550_PRINTER'),
      { printerName })
  }

  this.getConsumableInfoIn550Printer = function (printerName) {
    return invokeWsCommandAsync(GET,
      getSetting('WS_CMD_GET_CONSUMABLE_INFO_IN_550_PRINTER'),
      { printerName })
  }
  ...
  this.printLabel2Async = function (
    printerName, printParamsXml, labelXml, labelSetXml) {
    return invokeWsCommandAsync(POST, getSetting('WS_CMD_PRINT_LABEL2'), {
      printerName,
      printParamsXml,
  ...
  this.renderLabelAsync = function (
    labelXml, renderParamsXml, printerName) {
    return invokeWsCommandAsync(POST, getSetting('WS_CMD_RENDER_LABEL'), {
      labelXml,
      renderParamsXml,
