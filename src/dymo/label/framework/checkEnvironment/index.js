...
/**
 ...
 @export
 */
const checkEnvironment = function (onEnvironmentCheckedCallback, checkWebService) {
  const result = {};
  ...
  const onWebServiceFound = function () {
  ...
  const onLegacyPluginFound = function () {
    result.isBrowserSupported = true;
    result.isFrameworkInstalled = true;
    result.isWebServicePresent = false;
  ...
  const checkLegacyPlugins = function () {
    ...
    if (platform.indexOf('Win') != -1) {
      ...
      if ('ActiveXObject' in window) {
        ...
        result.isBrowserSupported = true;
        try {
          const framework = new ActiveXObject('DYMOLabelFrameworkIEPlugin.Plugin');
          ...
          else result.isFrameworkInstalled = true;
        } catch (e) {
        ...
        if (_findPlugin('application/x-dymolabel')) {
          ...
          result.isFrameworkInstalled = true;
        } else result.errorDetails = 'DYMO Label Framework Plugin is not installed';
      }
      ...
      const safariPluginFound = _findPlugin('application/x-dymolabel');

      if (safariPluginFound) {
        traceMsg('checkLegacyPlugins > safariPluginFound');
        // check version, should be at least 2.0
        const safariPlugin = _createSafariPlugin();
        if (safariPlugin.GetAPIVersion() >= '2.0') result.isFrameworkInstalled = true;
        else
          result.errorDetails =
        ...
        if (_findPlugin('application/x-npapi-dymolabel')) {
          ...
          const safariPlugin = _createMacNsapiPlugin();
        ...
        } else result.errorDetails = 'DYMO Label Plugin is not installed.';
      }
    } else result.errorDetails = 'The operating system is not supported.';
  };
